...
public class TphereCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!p.isOp() && !p.hasPermission("utility.tphere")) {
            p.sendMessage(PREFIX + ChatColor.RED + "You do not have permission to use this command.");
            return true;
        }
        if (args.length != 1) {
            p.sendMessage(PREFIX + ChatColor.RED + "Usage: /tphere <player>");
            return true;
        }
        ...
        p.sendMessage(PREFIX + ChatColor.GREEN + "Teleported " + target.getName() + " to you.");
        return true;
    }
}