...
public class TpaCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length != 1) {
            p.sendMessage(PREFIX + "Usage: /tpa <player>");
            return true;
        }
        ...
        TeleportRequestManager.sendRequest(p, target, false);
        p.sendMessage("§aTeleport request sent to §e" + target.getName());
        target.sendMessage("§e" + p.getName() + "§a wants to teleport to you. Use §6/tpaccept§a or §6/tpdeny");
